@php
    $approver1Id = old(
        ...
        $moduleConfig?->approvers->firstWhere('approval_order', 1)?->user_id
            ?? $moduleConfig?->approvers->firstWhere('role', \App\Models\InventoryModuleApprover::ROLE_APPROVER)?->user_id
    );
    $approver2Id = old('approver_2', $moduleConfig?->approvers->firstWhere('approval_order', 2)?->user_id);
    $technicalSupervisorId = old(
        'technical_supervisor',
        $moduleConfig?->approvers->firstWhere('role', \App\Models\InventoryModuleApprover::ROLE_TECHNICAL_SUPERVISOR)?->user_id
    );
@endphp
...
<div class="border border-gray-200 rounded-lg p-4 space-y-6" id="grn-approvers-fields">
    ...
    <div>
        ...
        <p class="text-xs text-gray-500 mb-2">
            If set, this person is an extra first approver on goods receive notes only. Leave unset to skip this step.
        </p>
        <input type="text"
               id="search-grn-technical-supervisor"
               placeholder="Search by name or email..."
               autocomplete="off"
               class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm text-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
        <div class="mt-2 space-y-1 max-h-48 overflow-y-auto rounded-md border border-gray-200 p-2">
            <label class="flex items-center gap-3 px-2 py-2 rounded-md hover:bg-gray-50 cursor-pointer grn-technical-supervisor-row"
                   data-name="none"
                   data-email="">
                <input type="radio"
                       name="technical_supervisor"
                       value=""
                       {{ empty($technicalSupervisorId) ? 'checked' : '' }}
                       class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                <span class="text-sm text-gray-500 italic">— None —</span>
            </label>
            @foreach($businessUsers as $user)
                <label class="flex items-center gap-3 px-2 py-2 rounded-md hover:bg-gray-50 cursor-pointer grn-technical-supervisor-row"
                    ...
                    <input type="radio"
                           name="technical_supervisor"
                           value="{{ $user->id }}"
                           {{ (string) $technicalSupervisorId === (string) $user->id ? 'checked' : '' }}
                           class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                    <span class="text-sm text-gray-900">
                        {{ $user->name }}
                        <span class="text-gray-500">({{ $user->email }})</span>
