...
<div class="min-h-screen bg-gray-50 py-6">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            ...
            <h2 class="mt-2 text-2xl font-bold text-gray-900">Edit service charge maturation</h2>
        </div>
        ...
        <div class="bg-white shadow sm:rounded-lg">
            <form action="{{ route('service-charge-maturation-periods.update', $serviceChargeMaturationPeriod) }}" method="POST" class="px-4 py-5 sm:p-6 space-y-6">
                @csrf
                @method('PUT')
                <div>
                    ...
                    <select name="business_id" id="business_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        @foreach($businesses as $business)
                            <option value="{{ $business->id }}" @selected(old('business_id', $serviceChargeMaturationPeriod->business_id) == $business->id)>{{ $business->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="payment_method" id="payment_method" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        @foreach($paymentMethods as $method)
                            ...
                            @endphp
                            <option value="{{ $method }}" @selected(old('payment_method', $serviceChargeMaturationPeriod->payment_method) === $method)>{{ $labels[$method] ?? $method }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="maturation_days" id="maturation_days" min="0" max="365" required
                           value="{{ old('maturation_days', $serviceChargeMaturationPeriod->maturation_days) }}"
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('description', $serviceChargeMaturationPeriod->description) }}</textarea>
                    @error('description')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="flex items-center">
                    <input type="checkbox" name="is_active" id="is_active" value="1" class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500" @checked(old('is_active', $serviceChargeMaturationPeriod->is_active))>
                    <label for="is_active" class="ml-2 text-sm text-gray-700">Active</label>
                </div>

                <div class="flex gap-3">
                    <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">Update</button>
