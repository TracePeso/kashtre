        ...
        @foreach($tabs as $tab)
            <li>
                <a href="{{ route($tab['route']) }}"
                   class="inline-block py-3 px-1 border-b-2 text-sm font-medium whitespace-nowrap {{ request()->routeIs($tab['route']) ? 'border-blue-600 text-blue-700' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    {{ $tab['label'] }}
                </a>
            </li>
        @endforeach
