...
<div class="min-h-screen bg-gray-50 py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-2xl font-bold text-gray-900">Expired escrow</h2>
        <p class="mt-1 text-sm text-gray-500">
            Stock held in expired escrow is not dispensable. Write-off clears it from the books.
            Also available from Monitor Stock.
        </p>
        ...
        <div class="mt-6 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            <div class="border-b border-gray-200 px-4 sm:px-6">
                <nav class="-mb-px flex flex-wrap gap-1" aria-label="Escrow tabs">
                    <a href="{{ route('inventory.escrow.index', ['tab' => 'expired']) }}"
                       @class([
                           'whitespace-nowrap border-b-2 px-4 py-3 text-sm font-medium transition',
                           'border-blue-600 text-blue-700' => $tab === 'expired',
                           'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700' => $tab !== 'expired',
                       ])>
                        Expired
                        @if ($expiredCount > 0)
                            <span class="ml-1 inline-flex min-w-[1.25rem] items-center justify-center rounded-full bg-amber-100 px-1.5 text-[11px] font-semibold text-amber-900">
                                {{ $expiredCount }}
                            </span>
                        @endif
                    </a>
                    <a href="{{ route('inventory.escrow.index', ['tab' => 'written-off']) }}"
                       @class([
                           'whitespace-nowrap border-b-2 px-4 py-3 text-sm font-medium transition',
                           'border-blue-600 text-blue-700' => $tab === 'written-off',
                           'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700' => $tab !== 'written-off',
                       ])>
                        Written off
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left font-medium text-gray-600">Store</th>
                                <th class="px-4 py-3 text-left font-medium text-gray-600">Item</th>
                                <th class="px-4 py-3 text-right font-medium text-gray-600">Written off</th>
                                <th class="px-4 py-3 text-right font-medium text-gray-600">Escrow after</th>
                                <th class="px-4 py-3 text-left font-medium text-gray-600">By</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @forelse($writeOffs as $row)
                                <tr>
                                    <td class="px-4 py-3 text-gray-700 whitespace-nowrap">{{ $row->committed_at?->format('d M Y H:i') ?? '—' }}</td>
                                    <td class="px-4 py-3 text-gray-900">{{ $row->store_name }}</td>
                                    <td class="px-4 py-3">
                                        <span class="font-medium text-gray-900">{{ $row->item_name }}</span>
                                        @if ($row->item_code)
                                            <span class="block text-xs text-gray-500">{{ $row->item_code }}</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-right tabular-nums text-gray-600">
                                        {{ rtrim(rtrim(number_format((float) $row->escrow_after, 2), '0'), '.') }}
                                    </td>
                                    <td class="px-4 py-3 text-gray-600">{{ $row->actor_name }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-8 text-center text-gray-500">
                                        No write-offs yet. Write off from the Expired tab to build this history.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
