<x-app-layout>
<div class="min-h-screen bg-gray-50 py-6" x-data="grnCreateForm(@js($items->map(fn ($i) => [
    'id' => $i->id,
    'name' => $i->name,
    'category' => $i->category,
    'code' => $i->code,
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="md:flex md:items-center md:justify-between mb-6">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Delivery note fields (columns B–L). Stock updates after approvers sign off.</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('inventory.receive.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
            ...
            <div class="bg-white shadow sm:rounded-lg p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label for="store_id" class="block text-sm font-medium text-gray-700">Receiving store</label>
                        <select name="store_id" id="store_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Lead time (days) <span class="text-gray-400 text-xs">col K</span></label>
                        <p class="mt-2 text-sm text-gray-900 font-medium tabular-nums" x-text="leadTimeDays"></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Entry by <span class="text-gray-400 text-xs">col L</span></label>
                        ...
                        <div class="border border-gray-200 rounded-lg p-4 relative">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="mt-2 text-sm text-gray-600" x-text="line.category || '—'"></p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Quantity (QTY) <span class="text-red-500">*</span></label>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">DUOM</label>
                                    <input type="text" :name="'lines[' + index + '][duom]'" x-model="line.duom"
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm">
                                </div>
                                <div>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">SUOM</label>
                                    <p class="mt-2 text-sm text-gray-600" x-text="line.suom || '—'"></p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Sale units per purchase unit (I)</label>
                                    <input type="number" step="0.0001" min="0.0001" :name="'lines[' + index + '][sale_units_per_purchase_unit]'" x-model.number="line.conversion" required
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Sale units purchased (J = I×C)</label>
                                    <p class="mt-2 text-sm font-semibold text-gray-900 tabular-nums" x-text="saleUnits(line).toLocaleString(undefined, {maximumFractionDigits: 4})"></p>
                                </div>
                            </div>
                        </div>
                    </template>
...
<script>
function grnCreateForm(items) {
    return {
        items,
        dateOfOrder: @json(old('date_of_order', now()->toDateString())),
        dateOfDelivery: @json(old('date_of_delivery', now()->toDateString())),
        lines: [{ item_id: '', category: '', suom: '', duom: '', quantity: 1, batch_number: '', expiry_date: '', purchase_price: 0, conversion: 1 }],
        get leadTimeDays() {
        ...
        addLine() {
            this.lines.push({ item_id: '', category: '', suom: '', duom: '', quantity: 1, batch_number: '', expiry_date: '', purchase_price: 0, conversion: 1 });
        },
        removeLine(index) {
