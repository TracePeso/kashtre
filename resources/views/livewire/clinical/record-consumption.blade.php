<div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
    ...
    <div class="mt-2">
        ...
        <input type="text" wire:model="justificationNote" placeholder="Why this item was taken from stock"
            class="w-full text-sm rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
        @error('justificationNote') <div class="text-[10px] text-red-600">{{ $message }}</div> @enderror
    </div>
            ...
            <tbody>
                @foreach ($recentEvents as $event)
                    <tr wire:key="ce-{{ $event->id }}" class="border-t border-gray-100 dark:border-gray-700">
                        <td class="py-1.5 text-gray-900 dark:text-gray-100">{{ $event->itemCode }}</td>
                        <td class="py-1.5 text-gray-700 dark:text-gray-300">{{ $event->quantity }}</td>
                        <td class="py-1.5 text-gray-700 dark:text-gray-300">{{ str_replace('_', ' ', $event->factToken) }}</td>
                        <td class="py-1.5 text-gray-700 dark:text-gray-300">
                            @if ($event->deliveryStatus)
                                <span class="text-[10px] px-1.5 py-0.5 rounded uppercase
                                    {{ $event->deliveryStatus === 'DISPATCHED' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' : 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400' }}">
                                    {{ $event->deliveryStatus }}
                                </span>
                            @elseif (is_null($event->physicalStockReduced))
                                <span class="text-gray-400">—</span>
                            @else
                                {{ $event->physicalStockReduced ? 'Yes' : 'No' }}
                            @endif
                        </td>
                        <td class="py-1.5 text-gray-500 dark:text-gray-400">{{ $event->recordedAt }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    {{-- Delivery status is Clinical's own honest word for whether the outbox
         fact this row queued has reached Main's Inventory — PENDING here is
         expected and correct today, not a bug: that hand-off is deliberately
         a human action on the existing Record Usage screen, not automatic
         (see the panel above Inventory → Record Usage, and
         ApiConsumptionGateway's class doc). --}}
</div>
